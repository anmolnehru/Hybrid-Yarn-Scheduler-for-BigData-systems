...
public class GenTable extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
     ...
     String inputDir=table;
     if(line.hasOption("input")) {
      inputDir = line.getOptionValue("input");
      }
        ...
        Path in = genInput(inputDir, table, scale, parallel);

        Path dsdgen = copyJar(new File("target/lib/dsdgen.jar"), inputDir);
        ...
        Job job = new Job(conf, "GenTable+"+inputDir+"_"+table+"_"+scale);
    ...
    public Path copyJar(File jar, String inputDir) throws Exception {
      ...
      Path dst = new Path(String.format("/tmp/%s_%s.jar",md5hex, inputDir));
    ...
    public Path genInput(String inputDir, String table, int scale, int parallel) throws Exception {
        long epoch = System.currentTimeMillis()/1000;

        Path in = new Path("/tmp/"+inputDir+"_"+table+"_"+scale+"-"+epoch);
        FileSystem fs = FileSystem.get(getConf());
